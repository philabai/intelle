...
export function RegulationOriginalPane({ regId, sourceUrl, hasCached }: Props) {
  ...
  // HTML source — sandboxed iframe to the cached signed URL. The
  // bucket is public-read but only contains regulator pages we already
  // mirror; sandbox blocks scripts so embedded analytics don't fire.
  return (
    <div className="rounded-md border border-card-border bg-background">
      <iframe
        src={state.signedUrl}
        title="Original regulation source"
